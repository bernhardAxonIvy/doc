...
|ivy-engine| supports Single Sign-on. The configuration depends on the given
infrastructure, but the concept looks always the same. 

The user is browsing the |ivy-engine| via a intermediate service - a
...
The |ivy-engine| will automatically log in the user when the user is known in
...
.. warning::
  If you enable SSO you have to ensure that the |ivy-engine| cannot be accessed 
...
You can integrate |ivy-engine| with every :ref:`Web Application Firewall
