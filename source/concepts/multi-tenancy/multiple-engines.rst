...
Axon Ivy Engine installation per tenant or per customer. Doing so brings the
...
- **Scaling**: A separate Axon Ivy Engine per tenant is simple to maintain and
  scales very well. On the other hand, if you run an Axon Ivy Cluster installation
...
- **Canary Deployments**: Introducing new features always carries risks. Instead
  ...
  wrong, only that group is affected. With a separate Axon Ivy Engine per tenant,
...
Axon Ivy Engine installations. We recommend using container environments such as
